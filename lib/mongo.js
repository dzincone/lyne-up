...
module.exports = {
    ...
    return leagues.find().then(function(leagues){
      return divisions.find().then(function(data){
        ...
        if(data != 0){
          ...
          res.render("lyneup/coach/join-league", {user: req.session.username, name: req.session.name, leagues: leagues})
        }
      })
    })
  },
  ...
  createTeam: function(req) {
    ...
    for(var i = 0; i < req.body.count; i++){
      ...
      promiseArray.push(players.create({first_name: req.body.first_name,
                      last_name: req.body.last_name,
                      jersey: req.body.number}).then(function(player){

                        return teams.findOneAndUpdate({team_name: req.body.team,
                                                      coach: req.session.username},
                                            {team_name: req.body.team,
                                            coach: req.session.username,
                                            assistant: req.body.assistant,
                                            $addToSet: {players: player._id}},
                                            {"upsert": true, "new": true})}).then(function(team){

                                    return divisions.findOneAndUpdate({_id:
